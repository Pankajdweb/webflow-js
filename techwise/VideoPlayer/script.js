 ...
 *
 * Rules:
 * - If `.tw-vp-thumbnail` already has a usable `src`, do nothing.
 * - Otherwise:
...
(function (window) {
  ...
  function ensureThumbnailEls(wrapper) {
    ...
    var container = wrapper.querySelector(".tw-vp-container");
    if (!container) return null;

    var holder = container.querySelector(".tw-vp-thumbnail-holder");
    if (!holder) {
      holder = document.createElement("div");
      holder.className = "tw-vp-thumbnail-holder";
      container.appendChild(holder);
    }

    var img = holder.querySelector("img.tw-vp-thumbnail");
    if (!img) {
      img = document.createElement("img");
      img.className = "tw-vp-thumbnail";
      img.alt = "";
  ...
  async function ensureThumbnailUrl(wrapper) {
    ...
    var url = wrapper.getAttribute("tw-data-video-url") || "";
    if (!url) return;
    ...
    try {
      ...
      wrapper.dataset.twVpThumbnailReady = "false";
    }
  }

  function hideThumbnail(wrapper) {
    if (!wrapper) return;
    wrapper.classList.add("tw-vp-thumb-hidden");
  }

  function showThumbnail(wrapper) {
    if (!wrapper) return;
    wrapper.classList.remove("tw-vp-thumb-hidden");
  }

  function initWrapper(wrapper) {
    if (!wrapper || wrapper._twVpThumbInitialized) return;
    wrapper._twVpThumbInitialized = true;

    ensureThumbnailEls(wrapper);

    // Visible on load, except autoplay (background hero).
    if (wrapper.hasAttribute("tw-data-autoplay")) hideThumbnail(wrapper);
  ...
  function initAll() {
    var wrappers = document.querySelectorAll(".tw-vp-wrapper");
    for (var i = 0; i < wrappers.length; i++) initWrapper(wrappers[i]);
  }

  // Event wiring from video-player.js
  document.addEventListener("tw-vp:init", function (e) {
    var wrapper = e && e.detail && e.detail.wrapper;
    if (wrapper) initWrapper(wrapper);
  });

  document.addEventListener("tw-vp:play", function (e) {
    var wrapper = e && e.detail && e.detail.wrapper;
    if (!wrapper) return;
    hideThumbnail(wrapper);
  });

  document.addEventListener("tw-vp:ended", function (e) {
    var wrapper = e && e.detail && e.detail.wrapper;
    if (!wrapper) return;
...
(function (window) {
  ...
  function describeEl(el) {
    try {
      ...
      var url = el.getAttribute && el.getAttribute("tw-data-video-url");
      if (url) s += ' [data-video-url="' + url + '"]';
      return s;
    } catch (e) { return "tw-vp-wrapper"; }
  }
  ...
  function parseAutoplayAttr(wrapper) {
    if (!wrapper.hasAttribute("tw-data-autoplay")) return false;
    var v = wrapper.getAttribute("tw-data-autoplay");
  ...
  function VideoPlayer(wrapper) {
    ...
    this.container = wrapper.querySelector(".tw-vp-container");
    if (!this.container) {
      // Required in your markup; we create it but log so you can fix HTML.
      error(wrapper, 'Missing required element: child ".tw-vp-container". Creating one automatically.');
      this.container = document.createElement("div");
      this.container.className = "tw-vp-container";
      wrapper.appendChild(this.container);
    }
    this.url = wrapper.getAttribute("tw-data-video-url") || "";
    ...
    this.showControls = !wrapper.hasAttribute("tw-data-hide-controls") && !this.autoplay;
    this.title = wrapper.getAttribute("tw-data-title") || wrapper.getAttribute("aria-label") || "Video";
  ...
  VideoPlayer.prototype.init = function () {
    ...
    this.wrapper.classList.add("tw-vp-min");
    if (this.source.type === "youtube" || this.source.type === "vimeo") {
      this.wrapper.classList.add("tw-vp-embed-cover");
    }
    this.wrapper.setAttribute("tabindex", "0");
    this.wrapper.classList.remove("tw-vp-started");
    this.wrapper.setAttribute("aria-label", this.title);

    emit(this.wrapper, "tw-vp:init", { wrapper: this.wrapper, player: this });
  ...
  VideoPlayer.prototype.buildControls = function () {
    ...
    wrap.className = "tw-vp-controls";
    wrap.innerHTML =
      '<div class="tw-vp-progress-wrapper" aria-label="Video progress" tabindex="0">' +
        '<div class="tw-vp-progress-bar">' +
          '<div class="tw-vp-progress-fill"></div>' +
          '<div class="tw-vp-progress-handle"></div>' +
        '</div>' +
      '</div>' +
      '<div class="tw-vp-controls-bar">' +
        '<button type="button" class="tw-vp-control-btn tw-vp-play-toggle" aria-label="Play">' +
          ICONS.play +
        '</button>' +
        '<span class="tw-vp-time">0:00 / 0:00</span>' +
        '<div class="tw-vp-spacer"></div>' +
        '<div class="tw-vp-volume-container">' +
          '<button type="button" class="tw-vp-control-btn tw-vp-volume-toggle" aria-label="Mute">' +
            ICONS.volumeOn +
          '</button>' +
          '<div class="tw-vp-volume-slider">' +
            '<div class="tw-vp-volume-fill" style="width:100%"></div>' +
          '</div>' +
        '</div>' +
        '<button type="button" class="tw-vp-control-btn tw-vp-fs-toggle" aria-label="Fullscreen">' +
          ICONS.fullscreen +
        '</button>' +
      '</div>';
    ...
    this.progressWrapper = wrap.querySelector(".tw-vp-progress-wrapper");
    this.progressFill = wrap.querySelector(".tw-vp-progress-fill");
    this.progressHandle = wrap.querySelector(".tw-vp-progress-handle");
    this.playBtn = wrap.querySelector(".tw-vp-play-toggle");
    this.timeLabel = wrap.querySelector(".tw-vp-time");
    this.volumeContainer = wrap.querySelector(".tw-vp-volume-container");
    this.volumeSlider = wrap.querySelector(".tw-vp-volume-slider");
    this.volumeFill = wrap.querySelector(".tw-vp-volume-fill");
    this.volumeBtn = wrap.querySelector(".tw-vp-volume-toggle");
    this.fsBtn = wrap.querySelector(".tw-vp-fs-toggle");
  ...
  VideoPlayer.prototype.createEmbed = function () {
    ...
    if (this.source.type === "youtube") {
      ...
      ov.className = "tw-vp-click-overlay";
          ...
          self.providerPlayer = new YT.Player(iframe, {
            events: {
              ...
              onStateChange: function (ev) {
                ...
                if (ev && ev.data === 0) emit(self.wrapper, "tw-vp:ended", { wrapper: self.wrapper, player: self });
    ...
    if (this.source.type === "vimeo") {
      ...
      ov2.className = "tw-vp-click-overlay";
      ...
      loadVimeoAPI().then(function (Vimeo) {
        try {
          ...
          self.providerPlayer.on("ended", function () {
            ...
            self.updateCenterButton();
            self.updateControls();
            emit(self.wrapper, "tw-vp:ended", { wrapper: self.wrapper, player: self });
          });
  ...
  VideoPlayer.prototype.play = function () {
    ...
    this.wrapper.classList.add("tw-vp-started");
    ...
    emit(this.wrapper, "tw-vp:play", { wrapper: this.wrapper, player: this });
  };
  ...
  function initAllPlayers() {
    ...
    for (var i = 0; i < wrappers.length; i++) {
      if (wrappers[i]._twVpInitialized) continue;
      wrappers[i]._twVpInitialized = true;
      try {
